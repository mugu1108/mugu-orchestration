...
> **重要**: スタイルガイド準拠チェックとlayout-fix検証を実施します。
...
| チェック項目 | 基準 | 重要度 |
|-------------|------|--------|
| タイトルにコロン | 使用禁止（「原則: 自律性」→「原則 自律性」） | **必須** |
| 感嘆符 | 使用禁止 | **必須** |
| 疑問符（本文） | 使用禁止（Q&Aスライドは除く） | **必須** |
| 装飾的絵文字 | 使用禁止（✅❌⚠️のみ許可） | **必須** |
| 派手な色 | red-600/green-600等は禁止 | **必須** |
...
| チェック項目 | 基準 | 重要度 |
|-------------|------|--------|
| アクセントカラー数 | 1スライド1-2色まで | 必須 |
| 許可カラー | グレースケール + #1B4565 + #3E9BA4 | 必須 |
| 禁止カラー使用 | red-600, green-600, blue-600等 | 必須 |
...
Slide Reviewer エージェント
    │
    ├── Phase 0: スタイルガイド準拠チェック【新規】
    │   ├── 禁止事項チェック（コロン、感嘆符、絵文字）
    │   ├── カラーパレット検証
    │   └── レイアウトパターン準拠確認
    │
    ├── Phase 1: フォーマットチェック
    ...
    │   └── 時間配分確認
    │
    ├── Phase 3: layout-fix検証【新規】
    │   ├── オーバーフロー検出（下部切れ）
    │   ├── 位置ずれ検出
    │   └── gap/margin/padding調整提案
    │
    ├── Phase 4: レポート生成
    │   ├── 問題点リスト
    │   ├── 重要度分類（必須/推奨）
...
| 問題 | 検出方法 | 修正アクション |
|------|---------|---------------|
| 下部切れ（オーバーフロー） | コンテンツ量測定 | gap/margin/padding値を削減 |
| 5項目以上の縦リスト | 項目数カウント | 2カラムグリッドに変換 |
| 位置ずれ | Tailwindクラス検証 | コンテナサイズ調整 |
