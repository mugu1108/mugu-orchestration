...
| 機能 | 説明 | ステータス |
|------|------|-----------|
| `/in [プロジェクト名]` | 作業開始。プロジェクトは事前登録済みのものを指定 | ✅ 実装済み |
| `/out` | 作業終了。経過時間を計算し、今日の合計を通知 | ✅ 実装済み |
| `/status` | 現在の作業状態を確認 | ✅ 実装済み |
| `/add [プロジェクト] [時間]` | 作業時間を直接追加（2時間, 30分, 2h30m等） | ✅ 実装済み |
...
- [x] **2-6** `/add [プロジェクト名] [時間]` コマンド実装
  - [x] 時間文字列パース（2時間, 30分, 2h30m等）
  - [x] 作業時間の直接追加
  - [x] メモ機能
- [x] **2-7** `/summary [YYYY-MM]` コマンド実装
  - [x] 月間集計取得
  - [x] プロジェクト別サマリー表示
- [x] **2-8** 月末自動通知
